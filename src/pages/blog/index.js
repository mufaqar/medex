          ...
          <div className='md:w-3/4 w-full'>
            <div className='grid grid-cols-1 gap-9'>
              {currentItems.map((post, idx) => {
                return (
                  <article key={idx} className='flex flex-col sm:flex-row blog-card overflow-hidden w-full border border-black/10 rounded-xl'>
                    <div className='relative flex-shrink-0 '>
                      <Link href={`/blog/${post.uri}`}>
                        <figure className="md:max-w-[440px] max-w-360px h-full">
                          <img src={post?.featuredImage?.node?.mediaItemUrl} alt="blog-image" className='object-cover h-full w-full object-left-top' />
                        </figure>
                      </Link>
                    </div>
                    <div className='flex flex-col justify-center py-5 px-5 sm:px-8 h-full overflow-hidden relative'>
                      <h3 className='font-semibold text-2xl lg:text-3xl mb-3.5 '>
                        <Link href={`/blog/${post.uri}`} className='hover:text-[#BF1800]'>
                          {post.title}
                        </Link>
                      </h3>
                      ...
                      <div className="flex justify-between">
                        <div className="entry-meta text-[13px] text-gray-500 flex">
                          <span className="post-on pr-2.5 relative flex items-center gap-1.5">
                            <svg stroke="currentColor" fill="currentColor" strokeWidth="0" viewBox="0 0 16 16" className="transition " height="1em" width="1em" xmlns="http://www.w3.org/2000/svg">
                        ...
                        <Link href={`/blog/${post.uri}`} className="text-gray-500 hover:text-[#BF1800] text-[13px] flex items-center gap-1.5">
                          Read More
                          <svg stroke="currentColor" fill="currentColor" strokeWidth="0" viewBox="0 0 16 16" className="false" height="1em" width="1em" xmlns="http://www.w3.org/2000/svg">
