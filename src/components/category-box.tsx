            ...
            <div className='grid gap-4 grid-cols-1 sm:grid-cols-3 2xl:gap-5  '>
                {Cate_Date.map((item, index) => (
                    <div key={index} className='mx-auto '>
                        <Link href={item.Cat_Link} className=' h-full flex justify-center relative overflow-hidden hover:opacity-90 border border-red-600 '>
                            <div className=''>
                                <Image src={item.featured} alt={item.featered_alt} className='object-cover w-full h-full' />
                            </div>
