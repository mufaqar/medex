    ...
    return (
        <>
            ...
            <header className='fixed top-0 left-0 right-0 bg-white/80 z-40 shadow-md'>
                <nav className="">
                    <div onClick={() => { setMegamenu(false), setDropdown(false) }} className="hidden md:flex flex-wrap justify-between items-center mx-auto px-4 md:px-6 py-2.5 bg-[#BF1800] relative z-50">
                    ...
                    <div className="flex flex-wrap justify-between items-center mx-auto px-4 md:px-6 py-2.5">
                        <Link href="/" className="flex items-center relative z-50">
                            <Image src={Logo} alt="logo" className='md:w-full w-1/2' onClick={() => { setMegamenu(false), setDropdown(false) }} />
                        </Link>
                        ...
                        <div className={`${navbar ? 'block' : 'hidden'} items-center justify-between w-full md:flex md:w-auto md:order-1 `}>
                            <ul className="relative z-50 flex flex-col mt-4 text-base uppercase font-medium montserrat md:flex-row md:space-x-8 md:mt-0">
                                <li onClick={() => { setMegamenu(false), setDropdown(false) }}>
                                ...
                                <li>
                                    <span className="flex items-center justify-between w-full py-2 pl-3 pr-4 font-medium text-gray-700 border-b border-gray-100 md:w-auto hover:bg-gray-50 text-base uppercase md:hover:bg-transparent md:border-0 md:hover:text-[#BF1800] md:p-0 ">
                                        <span onClick={() => { setMegamenu(false), setDropdown(false) }}>
                                            <Link href="/products">
                                                Products
                                            </Link>
                                        </span>
                                        <svg onClick={() => { setMegamenu(!megamenu), setDropdown(false) }} className="w-6 h-6 ml-1" fill="currentColor" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg"><path fillRule="evenodd" d="M5.293 7.293a1 1 0 011.414 0L10 10.586l3.293-3.293a1 1 0 111.414 1.414l-4 4a1 1 0 01-1.414 0l-4-4a1 1 0 010-1.414z" clipRule="evenodd"></path></svg>
                                    </span>
                                </li>
                                <li onClick={() => { setMegamenu(false), setDropdown(false) }}>
                                    <span className="flex items-center justify-between w-full py-2 pl-3 pr-4 font-medium text-gray-700 border-b border-gray-100 md:w-auto hover:bg-gray-50 text-base uppercase md:hover:bg-transparent md:border-0 md:hover:text-[#BF1800] md:p-0 ">
                                ...
                                <li>
                                    ...
                                    <ul onClick={() => { setMegamenu(false), setDropdown(false) }} className={`${dropdown ? 'block' : 'hidden'} bg-white md:absolute md:right-36 md:top-12 md:py-[15px] md:px-[30px] py-0 px-0`}>
                                        <li>
                    ...
                    <div className={`${megamenu ? 'block' : 'hidden'} mt-1 bg-white border-gray-200 shadow-sm border-y`}>
                        <div className="grid max-w-screen-xl px-4 py-5 mx-auto text-base text-gray-500  md:grid-cols-3 md:px-6">
                            ...
                            <Link onClick={() => { setMegamenu(false), setDropdown(false) }} href="/contact-us" className="p-8 text-left bg-local bg-gray-500 bg-center bg-no-repeat bg-cover rounded-lg bg-blend-multiply hover:bg-blend-soft-light" style={{ backgroundImage: 'url(/docs/images/dashboard-overview.png)' }}>
                ...
                <div onClick={() => { setMegamenu(false), setDropdown(false) }} className={`${megamenu || dropdown ? 'block' : 'hidden'} opacity-0 w-screen h-screen absolute top-0 -z-30`}></div>
            </header> </>
    )
}
